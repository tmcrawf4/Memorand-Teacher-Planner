...
public class EventsPanel extends JPanel {
    ...
    void jbInit() throws Exception {
        ...
        CurrentDate.addDateListener(new DateListener() {
            public void dateChange(CalendarDate d) {
                ...
                eventsTable.initTable(d, currentUserRank);
    ...
    void editEventB_actionPerformed(ActionEvent e) {
        ...
        dlg.topicField.setText(ev.getTopic());
        ...
        dlg.setSelectedVisibility(ev.getVisibility());
        dlg.setVisible(true);
        if (dlg.CANCELLED)
            return;
        ...
        AccountUtils.Rank visibility = dlg.getVisibilityChoice();
        EventsManager.removeEvent(ev);
        ...
        // subclass for topicField used to fetch the string for topic that is entered
        // in the dialog window
        String topic = dlg.topicField.getText();
        File file = dlg.getFile();
        if (dlg.noRepeatRB.isSelected())
   	        EventsManager.createEvent(CurrentDate.get(), hh, mm, text, topic, visibility, dlg.getFile());
        else {
	        updateEvents(dlg,hh,mm,text,topic, visibility);
	    }
	    saveEvents();
    }
    ...
    void newEventB_actionPerformed(ActionEvent e, String tasktext, Date startDate, Date endDate) {
    	...
    	if (dlg.noRepeatRB.isSelected())
    		EventsManager.createEvent(eventCalendarDate, hh, mm, text, topic, visibility, file);
    	else {
    		updateEvents(dlg,hh,mm,text, topic, visibility);
    	}
    	saveEvents();
    ...
    private void updateEvents(EventDialog dlg, int hh, int mm, String text, String topic, AccountUtils.Rank visibility) {
	    int rtype;
        int period;
        ...
        EventsManager.createRepeatableEvent(rtype, sd, ed, period, hh, mm, text, topic, dlg.workingDaysOnlyCB.isSelected(), visibility, dlg.getFile());
    }
